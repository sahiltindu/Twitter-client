...
const TwitterLayout: React.FC<TwitterlayoutProps> = (props) => {
  ...
  const handleLoginWithGoogle = useCallback(async(cred:CredentialResponse) => {
    ...
    console.log("this is google token",googleToken);
 ...
 <div className="grid grid-cols-12 h-screen w-screen sm:px-56">
      ...
      <div className="mt-1 text-xl pr-4">
        ...
        <div className="mt-5 px-3">
           <button className=" hidden sm:block   bg-[#1d9bf0] font-semibold text-lg py-2 px-4 rounded-full w-full">Tweet</button>
    ...
    { user &&(
     <div className="absolute bottom-5 flex gap-2 items-center bg-slate-800 px-3 py-2 rounded-full">
      ...
      <div className="col-span-0 sm:col-span-3 p-5">
        ...
        {
            user?.recommendedUsers?.map(el => ( <div className='flex items-center gap-3 mt-2' key={el?.id}>{el?.profileImageURL && <Image src={el?.profileImageURL} alt="user-image" className= "rounded-full" width={60} height={60} /> } 
